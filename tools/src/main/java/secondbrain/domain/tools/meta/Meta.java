...
public class Meta implements Tool<Void> {
    ...
    public List<RagDocumentContext<Void>> getContext(final Map<String, String> environmentSettings, final String prompt, List<ToolArgs> arguments) {
        ...
        final List<RagDocumentContext<Void>> ragDocs = filteredTools
                ...
                .collect(parallelToStream(tool -> {
                            ...
                            return Try.of(() -> tool.getContext(environmentSettings, prompt, toolArgs))
                                    .map(List::stream)
    ...
    private RagMultiDocumentContext<Void> callPrivate(
        ...
        final Try<List<RagDocumentContext<Void>>> contextResult = Try.of(() -> localStorage.getOrPutGeneric(
                getName(),
                getName(),
                Integer.toString(contextHashCode(environmentSettings, prompt, arguments)),
                parsedArgs.getCacheTtl(),
                List.class,
                RagDocumentContext.class,
                Void.class,
                () -> getContext(environmentSettings, prompt, arguments)).result());
    ...
    public int contextHashCode(final Map<String, String> environmentSettings, final String prompt, final List<ToolArgs> arguments) {
        final MetaConfig.LocalArguments parsedArgs = config.new LocalArguments(arguments, prompt, environmentSettings);
        return Seq.seq(getFilteredTools(parsedArgs)).foldLeft(17, (hash, tool) -> 31 * tool.contextHashCode(environmentSettings, prompt, arguments) + hash);
    }
