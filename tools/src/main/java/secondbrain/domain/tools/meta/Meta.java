...
public class Meta implements Tool<Void> {
    ...
    @Override
    public List<RagDocumentContext<Void>> getContext(final Map<String, String> environmentSettings, final List<String> prompts, List<ToolArgs> arguments) {
        ...
        final List<Tool<?>> filteredTools = getFilteredTools(parsedArgs);
        ...
        // Get the rag docs from each tool, converting to void context inside the parallel lambda
        // to avoid a sequential post-processing pass after the parallel stream
        final List<RagDocumentContext<Void>> ragDocs = filteredTools
                .stream()
                .collect(parallelToStream(tool -> {
                            logger.fine("Calling Meta tool " + tool.getName());
                            return Try.of(() -> tool.getContext(environmentSettings, List.of(prompt), toolArgs))
                                    .map(List::stream)
                ...
                .sorted((ragDoc1, ragdoc2) -> {
                    // A stable order of documents is critical for caching
                    if (ragDoc1.tool().equals(ragdoc2.tool())) {
                        return ragDoc1.document().compareTo(ragdoc2.document());
                    }

                    return ragDoc1.tool().compareTo(ragdoc2.tool());
                })
                .toList();
    ...
    private List<Tool<?>> getFilteredTools(final MetaConfig.LocalArguments parsedArgs) {
        // Use the parsed tool names
        final List<String> toolNames = parsedArgs.getToolNames();

        // Build a Set once for O(1) per-tool lookup instead of rebuilding a List for every tool in the filter
        final Set<String> toolNameSet = toolNames.stream()
                .map(String::toLowerCase)
                .collect(java.util.stream.Collectors.toSet());

        // Filter the tools to those included in the toolNames
        final List<Tool<?>> filteredTools = tools.stream()
                .filter(tool -> toolNameSet.contains(tool.getName().toLowerCase()))
    ...
    public RagMultiDocumentContext<Void> call(final Map<String, String> environmentSettings, final List<String> prompts, final List<ToolArgs> arguments) {
        ...
        return Try.of(() -> callPrivate(environmentSettings, prompts, arguments, parsedArgs))
                .map(result -> result.convertToRagMultiDocumentContextVoid())
                .get();
    }
    ...
    private RagMultiDocumentContext<Void> callPrivate(
        ...
        final Try<List<RagDocumentContext<Void>>> contextResult = Try.of(() -> localStorage.getOrPutGeneric(
                ...
                Void.class,
                () -> getContext(environmentSettings, prompts, arguments)).result());
...
class MetaConfig {
    ...
    @Inject
    @ConfigProperty(name = "sb.meta.autoGenerateKeywords")
    ...
    @Inject
    @ConfigProperty(name = "sb.meta.minimumContentLength", defaultValue = "0")
