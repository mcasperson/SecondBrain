...
public class PlanhatObjectives implements Tool<Void> {
    ...
    public int contextHashCode(final Map<String, String> environmentSettings, final List<String> prompts, final List<ToolArgs> arguments) {
        final PlanhatObjectivesConfig.LocalArguments parsedArgs = config.new LocalArguments(arguments, prompts, environmentSettings);
    ...
    private List<RagDocumentContext<Void>> getContextPrivate(
        ...
        final PlanhatObjectivesConfig.LocalArguments parsedArgs = config.new LocalArguments(arguments, List.of(prompt), environmentSettings);
