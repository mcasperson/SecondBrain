...
public record ToolCall(Tool<?> tool, ToolDefinition toolDefinition) {
    public RagMultiDocumentContext<?> call(final Map<String, String> context, final List<String> prompts, final Logger logger) {
        logger.log(Level.FINE, "Calling tool {0} with {1}", new Object[]{tool, toolDefinition});
        return tool.call(context, prompts, Objects.requireNonNullElse(toolDefinition.toolArgs(), List.of()));
    }
}
