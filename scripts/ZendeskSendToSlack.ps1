...
$ticketResult = Invoke-CustomCommand java "`"-Dsb.zendesk.excludedorgs=$( $env:EXCLUDED_ORGANIZATIONS )`" `"-Dsb.ollama.toolmodel=llama3.1`" `"-Dsb.ollama.model=llama3.1:70b`" `"-Dsb.ollama.contextlength=480000`" `"-Dstdout.encoding=UTF-8`" -jar $jarFile `"Given 1 days worth of ZenDesk tickets to recipient `"support@octopus.com`" provide a summary of the questions and problems in the style of a news article with up to 7 paragraphs. You must carefully consider each ticket when genering the summary. You will be penalized for showing category percentages. You will be penalized for including ticket IDs or reference numbers. Use concise, plain, and professional language. You will be penalized for using emotive or excited language. You will be penalized for including a generic final summary paragraph. You must only summarize emails that are asking for support. You will be penalized for summarizing emails that are not asking for support. You will be penalized for summarizing marketing emails. You will be penalized for attempting to answer the questions. You will be penalized for using terms like flooded, wave, or inundated.`" markdn"
