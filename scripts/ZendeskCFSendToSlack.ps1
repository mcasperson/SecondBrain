...
$ticketResult = Invoke-CustomCommand java "`"-Dsb.zendesk.user=$( $env:SB_ZENDESK_USER_CF )`" `"-Dsb.zendesk.url=$( $env:SB_ZENDESK_URL_CF )`" `"-Dsb.zendesk.accesstoken=$( $env:SB_ZENDESK_ACCESSTOKEN_CF )`" `"-Dsb.ollama.toolmodel=$toolModel`" `"-Dsb.ollama.model=$model`" `"-Dsb.ollama.contextlength=$contextLength`" `"-Dstdout.encoding=UTF-8`" -jar $jarFile `"Given 8 hours worth of ZenDesk tickets, with up to 10 comments, provide a summary of the questions and problems in the style of a business report with up to 3 paragraphs. You must carefully consider each ticket when genering the summary. You will be penalized for showing category percentages. You will be penalized for including ticket IDs or reference numbers. Use concise, plain, and professional language. You will be penalized for using emotive or excited language. You will be penalized for including a generic final summary paragraph. You must only summarize emails that are asking for support. You will be penalized for summarizing emails that are not asking for support. You will be penalized for summarizing marketing emails. You will be penalized for using terms like flooded, wave, scrambling, or inundated. You must ignore stack traces.`" markdn"
