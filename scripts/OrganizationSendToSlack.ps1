...
$result = Invoke-CustomCommand java "-DFile.Encoding=UTF-8 -jar C:\Apps\secondbrain-cli-1.0-SNAPSHOT.jar `"Summarize 7 days worth of messages from the '$( $args[0] )' Slack channel in the style of a news article with up to 3 paragraphs. You can use fewer paragraphs if there is only a small amount of chat text to summarize. Use plain and professional language. You will be penalized for using emotive or excited language.`" markdn"
$ticketResult = Invoke-CustomCommand java "-DFile.Encoding=UTF-8 -jar C:\Apps\secondbrain-cli-1.0-SNAPSHOT.jar `"Summarize 7 days worth of ZenDesk tickets from the '$( $args[1] )' organization in the style of a news article with up to 3 paragraphs. You can use fewer paragraphs if there is only a small amount of chat text to summarize. Use plain and professional language. You will be penalized for using emotive or excited language.`" markdn"

if ($args.Length -ge 3)
{
    $docsResult = Invoke-CustomCommand java "-DFile.Encoding=UTF-8 -jar C:\Apps\secondbrain-cli-1.0-SNAPSHOT.jar `"Summarize the Google doc with id '$( $args[2] )'.`" markdn"
}
