...
$result = Invoke-CustomCommand java "`"-Dstdout.encoding=UTF-8`" `"-Dsb.tools.force=SlackZenGoogle`" `"-Dsb.exceptions.printstacktrace=false`" `"-Dsb.slack.days=7`" `"-Dsb.google.doc=$($args[2])`" `"-Dsb.slack.channel=$($args[0])`" `"-Dsb.zendesk.organization=$($args[1])`" `"-Dsb.ollama.toolmodel=$toolModel`" `"-Dsb.ollama.model=$model`" -jar $jarFile `"Provide a summary of the slack messages and zendesk tickets associated with $($args[1]). Only reference the contents of the google document when it relates to content in the slack messages and zendesk tickets. You will be penalized for mentioning that there is no google document. If there are no zendesk tickets, say so. If there are no slack messages, say so. You will be penalized for referecing support tickets that were not explicilty provided. You will be penalized for referencing slak messages that were not explicilty provided. You will be penalized for saying that you will monitor for tickets or messages in future.`" markdn"
